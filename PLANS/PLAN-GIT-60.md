...
9. **Explicit font size written only on actual shrink (M3, arch-review):** `_apply_text_with_fit` sets `run.font.size` **only when** `applied < base` (the estimator actually shrank). When text fits at the base size AND the base was resolved via inheritance (tier ②/③, not an explicit schema `size_pt`), it does **not** write an explicit size — the placeholder keeps inheriting the layout/master styling (today's behaviour). This (a) preserves title/subtitle/body appearance on the common non-overflow path, (b) shrinks the `_set_body_text` refactor's regression surface from "all slides" to "only overflowing slides", and (c) honours AC2 in letter and spirit (no gratuitous reduction below the resolved base). Only `applied < base` ever produces an explicit size on a run.
