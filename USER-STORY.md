...
- Given a topic and page count, the output is a file that opens directly in PowerPoint.
- Every slide has a title and body content — no blank pages.
- The text, charts, and images in the file are all individually editable, not flattened into pictures.
...
**Acceptance criteria (Definition of Done):**
- After the outline is generated, it pauses and waits for my approval or edits before continuing.
- I can ask to add or remove slides, reorder them, or change titles.
- If I'm not available to review (e.g., running unattended), it can judge the outline itself and proceed.
...
**Acceptance criteria (Definition of Done):**
- Double-clicking a chart in PowerPoint lets me edit the values and the chart updates live.
- The chart's colors and styling match the rest of the deck — nothing looks out of place.
- Data labels are visible (e.g., each bar shows its exact number on top).
...
**Acceptance criteria (Definition of Done):**
- When I describe the image I want, a suitable picture appears on the right slide.
- The image's size and position are already set correctly — no manual dragging needed.
- If no suitable image can be found, the slide still generates without errors.
...
**Acceptance criteria (Definition of Done):**
- Every slide has a full spoken script — actual sentences I can read aloud, not dry bullet summaries.
- The script is visible in PowerPoint's Presenter View (the audience doesn't see it).
- Each script includes: the key point for that slide, a natural transition line to the next slide, and at least one likely audience question to be ready for.
...
**Acceptance criteria (Definition of Done):**
- Unless I explicitly ask for a custom closing, the last slide is automatically "Thank You."
- I never have to remember to say "add a closing slide" — it just happens.
- The closing slide's design matches the template, so it doesn't look tacked on.
