...
>
> **Revision 10 (pre-US-4.2, issue #60 — planned):** US-4.2 remains 🟡 Partial; a plan is locked in `PLANS/PLAN-GIT-60.md` (reactive text-fitting via a pure `text_fit.py` estimator + a sidecar `<output>.render.json` carrying the `font_size_adjusted` flag). Architecture review (APPROVE-WITH-CHANGES) incorporated: M1 (base-size resolution chain → schema `size_pt` → **layout sample-run** → conservative role ceiling body **14** / title **28** / subtitle **18**; AC2 re-framed to "≤ resolved base"), M2 (inter-paragraph spacing reserve in the height estimate), M3 (explicit `run.font.size` written **only on actual shrink**, else inheritance preserved). **Finding C1 is deferred by design** (see the US-4.2 caveat below): AC1's hard "no overflow" guarantee is **not verifiable** without a layout engine, so it is delivered best-effort this round and a full oracle (LibreOffice headless render) is left to a follow-up; AC1 stays unchecked. AC2/AC3 are on track to be Met.
...
> **Delivered (issue #60, `PLANS/GIT-60`):** the reactive layer is a pure `text_fit.py` estimator (−2pt steps to an 8pt floor) + a sidecar `<output>.render.json` carrying the `font_size_adjusted` flag (AC3 ✅). Body base size is now template-derived (M1 chain: schema `size_pt` → layout sample-run → conservative role ceiling body 14 / title 28 / subtitle 18), replacing the `Pt(14)`/`Pt(12)` hardcode (AC2 ✅, re-framed to "≤ resolved base"). An explicit `run.font.size` is written only on actual shrink (M3); an auto-grow guard prevents false-shrinking on short-base-height placeholders (titles). Inter-paragraph spacing is reserved in the height estimate (M2). **AC1 caveat (architecture-review C1, deferred by decision):** python-pptx has **no layout/text-measurement engine**, so AC1's hard guarantee — "no generated slide has text overflowing its bounding box" — **cannot be verified** by the engine alone. It is delivered **best-effort** this round (conservative estimator + `word_wrap=True`, which is a **horizontal-only** backstop and does **not** prevent vertical overflow). A full oracle (e.g. a LibreOffice headless render-to-image check) is **out of scope** and left to a follow-up; consequently **AC1 stays unchecked** until that verification path exists.

> **Pre-existing defect (NOT US-4.2):** the bundled `template.pptx` (pre-templated in US-4.1) mis-fingerprints body/picture placeholders — its layout fingerprints show only `TITLE`/`SUBTITLE`, never `OBJECT`/`PICTURE` — so `content_slide` / `two_content_slide` / `comparison_slide` / `content_image_slide` do not resolve and never reach the body-fitting path. This blocks end-to-end body-fitting verification on the bundled template and is tracked separately as **issue #61**.
