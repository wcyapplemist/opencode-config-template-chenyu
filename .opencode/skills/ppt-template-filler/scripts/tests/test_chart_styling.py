...
class TestChartFonts:
    def test_chart_global_font_is_calibri(self, bar_chart_data, output_path):
    ...
    def test_legend_font_is_calibri(self, bar_chart_data, output_path):
        generate_ppt_from_data([bar_chart_data], output_path=output_path)
        chart = _get_chart(output_path)
        assert chart.legend.font.name == EXPECTED_FONT

    def test_data_labels_font_is_calibri(self, bar_chart_data, output_path):
        generate_ppt_from_data([bar_chart_data], output_path=output_path)
        chart = _get_chart(output_path)
        assert chart.plots[0].data_labels.font.name == EXPECTED_FONT

    def test_value_axis_tick_font_is_calibri(self, bar_chart_data, output_path):
        generate_ppt_from_data([bar_chart_data], output_path=output_path)
        chart = _get_chart(output_path)
        assert chart.value_axis.tick_labels.font.name == EXPECTED_FONT

    def test_category_axis_tick_font_is_calibri(self, bar_chart_data, output_path):
        generate_ppt_from_data([bar_chart_data], output_path=output_path)
        chart = _get_chart(output_path)
...
class TestChartColors:
    def test_bar_series_uses_theme_color(self, bar_chart_data, output_path):
        generate_ppt_from_data([bar_chart_data], output_path=output_path)
        chart = _get_chart(output_path)
        series = chart.plots[0].series[0]
        assert series.format.fill.fore_color.rgb == EXPECTED_COLORS[0]

    def test_line_multi_series_colors(self, line_chart_data, output_path):
    ...
    def test_data_label_text_color(self, bar_chart_data, output_path):
        generate_ppt_from_data([bar_chart_data], output_path=output_path)
        chart = _get_chart(output_path)
        labels = chart.plots[0].data_labels
        assert labels.font.color.rgb == EXPECTED_TEXT_COLOR

    def test_legend_text_color(self, bar_chart_data, output_path):
        generate_ppt_from_data([bar_chart_data], output_path=output_path)
        chart = _get_chart(output_path)
...
class TestChartGridlines:
    def test_value_axis_has_major_gridlines(self, bar_chart_data, output_path):
        generate_ppt_from_data([bar_chart_data], output_path=output_path)
        chart = _get_chart(output_path)
        assert chart.value_axis.has_major_gridlines is True

    def test_gridline_color_matches_theme(self, bar_chart_data, output_path):
        generate_ppt_from_data([bar_chart_data], output_path=output_path)
        chart = _get_chart(output_path)
...
class TestChartNumberFormats:
    def test_pie_data_label_percentage_format(self, pie_chart_data, output_path):
        generate_ppt_from_data([pie_chart_data], output_path=output_path)
        chart = _get_chart(output_path)
        labels = chart.plots[0].data_labels
        assert labels.number_format == "0%"

    def test_bar_data_label_value_format(self, bar_chart_data, output_path):
        generate_ppt_from_data([bar_chart_data], output_path=output_path)
        chart = _get_chart(output_path)
        labels = chart.plots[0].data_labels
        assert labels.number_format == "#,##0.0"

    def test_custom_value_format(self, output_path):
        data = {
