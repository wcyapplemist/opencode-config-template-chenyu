...
class TestBackwardCompat:
    def test_title_slide_still_works(self, output_path):
    ...
    def test_content_slide_still_works(self, output_path):
        data = [{
    ...
    def test_closing_slide_still_works(self, output_path):
    ...
    def test_section_header_still_works(self, output_path):
        data = [{"slide_type": "section_header_slide", "title": "Section 1"}]
        generate_ppt_from_data(data, output_path=output_path)
        prs = Presentation(output_path)
        assert len(prs.slides) == 1

    def test_mixed_deck_without_charts(self, output_path):
        data = [
    ...
    def test_mixed_deck_with_charts(self, mixed_deck, output_path):
        generate_ppt_from_data(mixed_deck, output_path=output_path)
        prs = Presentation(output_path)
    ...
    def test_notes_preserved_on_all_types(self, output_path):
        data = [
    ...
    def test_unknown_slide_type_skipped(self, output_path):
        data = [
