...
The source can be:
  - A slide file (e.g., slide2.xml) - duplicates slide
...
Prints <p:sldId> element to add to presentation.xml.
"""
...
def _add_to_presentation_rels(unpacked_dir: Path, dest: str) -> str:
    ...
    rids = [int(m.group(1)) for m in re.findall(r'Id="rId(\d+)"', pres_rels)]
...
def _get_next_slide_id(unpacked_dir: Path) -> int:
    ...
    slide_ids = [int(m.group(1)) for m in re.findall(r'<p:sldId[^>]*id="(\d+)"', pres_content)]
    return max(slide_ids) + 256 if slide_ids else 256
...
if __name__ == "__main__":
    ...
    if source_type == "layout" and layout_file:
